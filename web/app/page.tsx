...
const platformModules = ["Overview", "Control"];
const organizationModules = ["Overview", "Control"];
  ...
  return (
    <div className="shell">
      ...
      <main>
        <header>
          <div>
            <p className="eyebrow">{application?.slug ?? organization?.slug ?? "INSTALLATION"}</p>
            <h1>{section}</h1>
          </div>
          <button className="outline" onClick={() => void loadOrganizations()}>
            Refresh
          </button>
        </header>
  ...
  return (
    <main className="setup">
      <section>
        ...
        <h1>Return to control.</h1>
        <p>Use an operator credential. Control-plane accounts remain separate from application users.</p>
        {message && <Toast message={message} onDismiss={() => setMessage("")} />}
        ...
        <div className="login-methods" aria-label="Operator sign-in method">
          <button type="button" className={method === "email" ? "active" : ""} disabled={!available} onClick={() => setMethod("email")}>Email code or link</button>
          <button type="button" className={method === "password" ? "active" : ""} onClick={() => setMethod("password")}>Password</button>
        </div>
        {method === "email" && available && <form onSubmit={submit}>
          {challenge ? (
            <label>
        ...
        <form onSubmit={(event) => void acceptInvitation(event)}>
          <p className="eyebrow">ORGANIZATION INVITATION</p>
          <p>Accept a one-time organization credential to create or update your operator account and sign in.</p>
          <label>
            Invitation credential
            <input required name="invitation_token" type="password" autoComplete="off" />
          </label>
          <label>
            Display name
...
function Workspace({
  ...
  if (section === "Control")
    return <ControlPlane organization={organization} setMessage={setMessage} />;
  if (!application)
    return (
      <>
        <section className="hero">
          ...
          <h2>
            One boundary.
            <br />
            <em>Every essential.</em>
          </h2>
          <p>
            Choose an organization and create or select an application to manage
            application identity, access, billing, and delivery.
          </p>
        </section>
        {organization ? (
          <ApplicationPicker
            ...
            <section className="cards">
              {organizations.map((org) => (
                <article key={org.id}>
                  ...
                  <h3>{org.name}</h3>
                  <p>{org.slug}</p>
                  {org.retired_at ? <button onClick={() => void setOrganizationRetirement(org, true)}>Restore organization</button> : <>
                    <button onClick={() => setOrganization(org)}>Open organization</button>
                    <button className="danger-action" onClick={() => void setOrganizationRetirement(org, false)}>Retire organization</button>
                  </>}
                </article>
              ))}
            </section>
  ...
  if (section === "Overview") return <Dashboard application={application} setMessage={setMessage} onRenamed={(updated) => {
    ...
    <>
      ...
      <div className="resource-view" key={selectedResource.path}>
        ...
        <section className="table">
          <div className="table-head"><span>{selectedResource.label}</span><span>{items.length} records</span></div>
...
function ResourceRow({ item, resource, application, setMessage, onInspect, onChanged }: { item: Record<string, unknown>; resource: string; application: Application; setMessage: (value: string) => void; onInspect: () => Promise<void>; onChanged: () => void }) {
  ...
  const actions: { label: string; method?: "POST" | "DELETE"; suffix: string; body?: unknown }[] = [];
  if (resource === "users") actions.push({ label: "Verify org", suffix: `users/${id}/verify-organization`, body: {} }, { label: item.status === "suspended" ? "Restore" : "Suspend", suffix: `users/${id}/${item.status === "suspended" ? "restore" : "suspend"}`, body: { reason: "operator action" } });
  if (resource === "billing/providers") actions.push({ label: "Verify", suffix: `billing/providers/${id}/verify`, body: {} }, { label: "Reconcile", suffix: `billing/providers/${id}/reconciliation-runs`, body: {} });
  ...
  if (resource === "local-entitlement-requests" && item.status === "pending") actions.push({ label: "Approve", suffix: `local-entitlement-requests/${id}/approve`, body: {} }, { label: "Reject", suffix: `local-entitlement-requests/${id}/reject`, body: { reason: "operator rejected" } });
  if (resource === "local-entitlement-requests" && ["rejected", "canceled"].includes(String(item.status))) actions.push({ label: "Reopen", suffix: `local-entitlement-requests/${id}/reopen`, body: { reason: "operator reopened for review" } });
  if (resource === "entitlements") actions.push({ label: item.revoked_at ? "Restore" : "Revoke", suffix: `entitlements/${id}/${item.revoked_at ? "restore" : "revoke"}`, body: { reason: item.revoked_at ? "operator restored" : "operator revoked" } });
  if (resource === "notification-templates" && !item.inherited) actions.push({ label: item.status === "draft" ? "Publish" : "Archive", suffix: `notification-templates/${id}/${item.status === "draft" ? "publish" : "archive"}`, body: {} });
  if (resource === "notifications" && ["failed", "dead"].includes(String(item.status))) actions.push({ label: "Retry", suffix: `notifications/${id}/retry`, body: {} });
  if (resource === "webhook-deliveries" && item.status !== "delivered") actions.push({ label: "Replay", suffix: `webhook-deliveries/${id}/replay`, body: {} });
  if (resource === "billing/provider-events") actions.push({ label: "Replay", suffix: `billing/provider-events/${id}/replay`, body: {} });
  if (resource === "sender-identities" && !item.is_default) actions.push({ label: "Make default", suffix: `sender-identities/${id}/default`, body: {} });
  if (resource === "roles" && !item.built_in) actions.push({ label: "Delete", method: "DELETE", suffix: `roles/${id}` });
  if (resource === "role-assignments") actions.push({ label: "Remove", method: "DELETE", suffix: `role-assignments/${id}` });
  if (resource === "workspaces") actions.push({ label: "Retire", method: "DELETE", suffix: `workspaces/${id}` });
  if (resource === "webhooks" && !item.disabled_at) actions.push(
    { label: "Send test", suffix: `webhooks/${id}/test`, body: {} },
    { label: "Rotate secret", suffix: `webhooks/${id}/rotate-secret`, body: {} },
    { label: "Disable", method: "DELETE", suffix: `webhooks/${id}` },
  );
  if (resource === "event-types" && item.source === "application" && item.status === "active") actions.push({ label: "Archive", method: "DELETE", suffix: `event-types/${id}` });
  ...
  return <article><div><strong>{title}</strong><small>{detail}</small></div><div className="row-actions">{resourceDetailPath(resource, id) && <button disabled={pendingAction !== ""} onClick={() => void inspect()}>{pendingAction === "Details" ? "Loading..." : "Details"}</button>}{actions.map((action) => <button disabled={pendingAction !== ""} key={action.label} onClick={() => void act(action)}>{pendingAction === action.label ? `${action.label}...` : action.label}</button>)}<code>{id}</code></div></article>;
}
...
function emailPreviewDocument(body: string) {
  return `<!doctype html><html><head><meta charset="utf-8"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; img-src data: cid:; style-src 'unsafe-inline'"><style>body{margin:0;padding:28px;color:#17221c;background:#fff;font:16px/1.55 Georgia,serif}a{color:#176b4a}h1,h2,h3{line-height:1.1}hr{border:0;border-top:1px solid #d8d4c8}</style></head><body>${body}</body></html>`;
}
  ...
  return <div className="template-composer">
    ...
    <div className="template-workbench">
      ...
      <aside className="template-codebook"><div><p className="eyebrow">AVAILABLE CODES</p><h4>Insert into {activeField === "html" ? "email body" : activeField}</h4><p>Click a code to place it at the cursor. User codes require <code>user_id</code> when sending.</p></div><div className="code-list">{allVariables.map((variable) => <button type="button" key={variable.key} onClick={() => insertCode(variable.key)}><span><strong>{variable.label}</strong><code>{`{{${variable.key}}}`}</code></span><small>{variable.description}</small><em>{variable.availability === "always" ? "Always available" : variable.availability === "user" ? "Requires user_id" : "Application supplied"} · Example: {String(variable.sample)}</em></button>)}</div>
        ...
        {customVariables.length > 0 && <div className="custom-code-summary">{customVariables.map((variable) => <span key={variable.key}><code>{variable.key}</code><button type="button" aria-label={`Remove ${variable.label}`} onClick={() => setCustomVariables((current) => current.filter((item) => item.key !== variable.key))}>Remove</button></span>)}</div>}
      </aside>
    </div>
...
function Dashboard({ application, setMessage, onRenamed }: { application: Application; setMessage: (value: string) => void; onRenamed: (application: Application) => void }) {
  ...
  useEffect(() => {
    let active = true;
    Promise.all([
      api.request<Record<string, unknown>>("GET", `/v1/control/applications/${application.id}/statistics`),
      api.request<Application>("GET", `/v1/control/applications/${application.id}`),
    ]).then(([statisticsValue, applicationValue]) => {
      if (!active) return;
      setStatistics(statisticsValue);
      setCurrent(applicationValue);
  ...
  async function rename(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage(readError(renameError));
    } finally {
      setRenaming(false);
    }
  }
...
function ApplicationPicker({ organization, setApplication, setMessage, reloadBoundaries, onOrganizationChanged }: {
  ...
  return <section className="boundary-picker">
    ...
    {loading ? <div className="empty">Loading applications...</div> : <div className="application-list">{applications.map((item) => <div key={item.id}>{item.retired_at ? <button onClick={() => void setApplicationRetirement(item.id, true)}><strong>{item.name}</strong><small>Restore retired application</small></button> : <><button onClick={() => setApplication(item)}><strong>{item.name}</strong><small>{item.slug}</small></button><button className="danger-action" onClick={() => void setApplicationRetirement(item.id, false)}>Retire</button></>}</div>)}</div>}
...
function ControlPlane({ organization, setMessage }: {
  organization: Organization | null;
  setMessage: (value: string) => void;
}) {
  const [members, setMembers] = useState<Record<string, unknown>[]>([]);
  ...
  useEffect(() => {
    ...
    Promise.all([
      api.request<Page<Record<string, unknown>>>("GET", "/v1/control/installation/operators"),
    ]).then(([operatorPage]) => {
  ...
  }, [organization, refresh, setMessage]);
  useEffect(() => {
    if (!organization) {
      setMembers([]);
      setInvitations([]);
  ...
  return <>
    <section className="hero compact"><p className="eyebrow">{organization ? "ORGANIZATION CONTROL" : "INSTALLATION CONTROL"}</p><h2>{organization ? organization.name : "Operators, not app users."}</h2><p>{organization ? "Organization roles govern every application inside this organization without creating application-user identities." : "Installation roles govern the whole deployment. Control-plane identities use an audience separate from application users."}</p></section>
    {!organization && (installationAccess === "loading" ? <div className="empty">Loading installation access...</div> : installationAccess === "unavailable" ? <div className="empty">Installation operators and SMTP are visible only to installation operators.</div> : <section className="control-scope">
      ...
      <ControlTable title="Installation operators" items={installationOperators} renderActions={(item) => <>{["auditor", "admin", "owner"].filter((role) => role !== item.role).map((role) => <button key={role} onClick={() => void action("PATCH", `/v1/control/installation/operators/${String(item.id)}`, { role })}>Make {role}</button>)}<button onClick={() => void action("DELETE", `/v1/control/installation/operators/${String(item.id)}`)}>Remove installation role</button></>} />
      <ManagementAPISettings setMessage={setMessage} />
      <InstallationTemplateSettings setMessage={setMessage} />
    </section>)}
    {organization && <section className="control-scope">
      ...
      <>
        <OrganizationPolicySettings organization={organization} setMessage={setMessage} />
        ...
        <ControlTable title={`${organization.name} operators`} items={members} renderActions={(item) => <>{["member", "auditor", "admin", "owner"].filter((role) => role !== item.role).map((role) => <button key={role} onClick={() => void action("PATCH", `/v1/control/organizations/${organization.id}/members/${String(item.id)}`, { role })}>Make {role}</button>)}<button onClick={() => void action("DELETE", `/v1/control/organizations/${organization.id}/members/${String(item.id)}`)}>Remove</button></>} />
        <ControlTable title="Organization invitations" items={invitations} renderActions={(item) => item.status === "pending" ? <><button onClick={() => void action("POST", `/v1/control/organizations/${organization.id}/invitations/${String(item.id)}/resend`, {})}>Resend</button><button onClick={() => void action("DELETE", `/v1/control/organizations/${organization.id}/invitations/${String(item.id)}`)}>Revoke</button></> : null} />
      </>
    </section>}
    <ProviderSettings basePath={organization ? `/v1/control/organizations/${organization.id}` : "/v1/control/installation"} scope={organization ? "organization" : "installation"} setMessage={setMessage} />
    <section className="control-scope">
      <div className="scope-heading"><p className="eyebrow">YOUR ACCESS</p><h3>Operator sessions</h3></div>
      <ControlTable title="Your operator sessions" items={sessions} renderActions={(item) => item.revoked_at ? null : <button onClick={() => void action("DELETE", `/v1/control/auth/sessions/${String(item.id)}`)}>Revoke</button>} />
      <button className="danger-action" onClick={() => void action("POST", "/v1/control/auth/logout-all", {})}>Log out every operator session</button>
    </section>
  </>;
}
...
function ManagementAPISettings({ setMessage }: { setMessage: (value: string) => void }) {
  ...
  return <section className="management-api-panel">
    ...
    <ControlTable title="Management clients" items={clients} renderActions={(item) => !status.can_manage || item.disabled_at ? null : <><button disabled={busy !== ""} onClick={() => void clientAction("POST", String(item.id))}>{busy === item.id ? "Rotating..." : "Rotate secret"}</button><button disabled={busy !== ""} onClick={() => void clientAction("DELETE", String(item.id))}>Disable</button></>} />
  </section>;
}
...
function InstallationTemplateSettings({ setMessage }: { setMessage: (value: string) => void }) {
  ...
  return <section className="installation-templates">
    ...
    {loading ? <LoadingState label="Loading installation email templates" /> : <section className="table"><div className="table-head"><span>Default templates</span><span>{templates.length} versions</span></div>{templates.map((template) => <article key={String(template.id)}><div><strong>{String(template.key)}</strong><small>{String(template.status)} · version {String(template.version)}{template.system_managed ? " · built in" : ""}</small></div><div className="row-actions"><button disabled={busy !== ""} onClick={() => void inspect(template)}>{busy === template.id ? "Loading..." : "Edit"}</button>{template.status === "draft" && <button disabled={busy !== ""} onClick={() => void publish(template)}>Publish</button>}</div></article>)}</section>}
...
function ProviderSettings({ basePath, scope, setMessage }: { basePath: string; scope: "installation" | "organization" | "application"; setMessage: (value: string) => void }) {
  ...
  useEffect(() => {
    ...
    ]).then(([auth, smtp, billing]) => {
      setAuthProviders(auth.items);
      setSMTPProviders(smtp.items);
      setBillingProviders(billing.items);
    }).catch((error) => setMessage(readError(error)));
  }, [basePath, refresh, setMessage]);
  async function submitProvider(event: FormEvent<HTMLFormElement>, kind: "google" | "apple" | "smtp" | "stripe") {
    event.preventDefault();
    const target = event.currentTarget;
    const form = new FormData(target);
    setBusy(kind);
    try {
      ...
      target.reset();
      setMessage(`${kind === "smtp" ? "SMTP" : kind === "stripe" ? "Stripe" : `${titleCase(kind)} login`} provider saved at the ${scope} scope.`);
      setRefresh((value) => value + 1);
    } catch (error) { setMessage(readError(error)); }
    finally { setBusy(""); }
  }
  async function disable(kind: "auth" | "notification" | "billing", provider: Record<string, unknown>) {
    ...
    try {
      await api.request("DELETE", `${basePath}/${kind === "auth" ? "auth/providers" : kind === "notification" ? "notification-providers" : "billing/providers"}/${identity}`);
      setMessage("Provider disabled. Children will resolve the next available inherited provider.");
      setRefresh((value) => value + 1);
    } catch (error) { setMessage(readError(error)); }
    finally { setBusy(""); }
  }
  async function verify(kind: "notification" | "billing", provider: Record<string, unknown>) {
    const providerScope = String(provider.scope ?? scope);
    if (providerScope !== scope) {
    ...
    try {
      await api.request("POST", `${basePath}/${kind === "notification" ? "notification-providers" : "billing/providers"}/${identity}/verify`, {});
      setMessage(`${kind === "notification" ? "SMTP" : "Stripe"} provider verified successfully.`);
      setRefresh((value) => value + 1);
    } catch (error) { setMessage(readError(error)); }
    finally { setBusy(""); }
  }
  async function toggleInheritance(kind: "auth" | "notification" | "billing", provider: Record<string, unknown>, inheritable: boolean) {
    const providerScope = String(provider.scope ?? scope);
    ...
    try {
      await api.request("PATCH", `${basePath}/${kind === "auth" ? "auth/providers" : kind === "notification" ? "notification-providers" : "billing/providers"}/${identity}`, { inheritable });
...
function ProviderCards({ items, currentScope, busy, onToggleInheritance, onVerify, onDisable }: { items: Record<string, unknown>[]; currentScope: string; busy: string; onToggleInheritance?: (item: Record<string, unknown>, inheritable: boolean) => void; onVerify?: (item: Record<string, unknown>) => void; onDisable: (item: Record<string, unknown>) => void }) {
  if (items.length === 0) return <div className="provider-empty">No provider resolves at this scope.</div>;
  return <div className="provider-cards">{items.map((item, index) => { const scope = String(item.scope ?? currentScope); const identity = String(item.id ?? item.provider); const local = scope === currentScope; return <article key={String(item.id ?? `${item.provider}-${index}`)}><div><strong>{String(item.name ?? item.provider)}</strong><span className={`provider-source ${local ? "local" : "inherited"}`}>{local ? `${titleCase(scope)} provider` : `Inherited from ${scope}`}</span></div><code>{String(item.client_id ?? item.sender_email ?? item.public_id ?? "configured")}</code><small>{item.verified_at ? "Verified" : item.status ? `Status: ${String(item.status)}` : item.inheritable ? "Available to children" : scope === "application" ? "Application only" : "This scope only"}</small>{local && currentScope !== "application" && onToggleInheritance && <label className="provider-global-toggle"><input type="checkbox" checked={Boolean(item.inheritable)} disabled={busy !== ""} onChange={(event) => onToggleInheritance(item, event.currentTarget.checked)} /><span>{busy === `inherit-${identity}` ? "Updating availability..." : currentScope === "installation" ? "Global default for organizations and applications" : "Available to applications in this organization"}</span></label>}{local && !item.disabled_at && <div className="provider-card-actions">{onVerify && <button disabled={busy !== ""} onClick={() => onVerify(item)}>{busy === `verify-${identity}` ? "Verifying..." : "Verify"}</button>}<button disabled={busy !== ""} onClick={() => onDisable(item)}>{busy === `disable-${identity}` ? "Disabling..." : "Disable"}</button></div>}</article>; })}</div>;
}

function providerDisplayName(kind: "auth" | "notification" | "billing", provider: Record<string, unknown>) {
