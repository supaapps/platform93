...
refresh credentials in Expo SecureStore and completes Google or Apple login through
an Expo WebBrowser authentication session.
...
The Google and Apple provider callbacks remain on Platform93. Platform93 redirects
the app only with a short-lived, single-use exchange credential, which the adapter
immediately exchanges for the application session.
